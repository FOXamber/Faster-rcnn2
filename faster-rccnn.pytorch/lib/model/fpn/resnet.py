...
class resnet(_FPN):
  ...
  def _init_modules(self):
    ...
    # stride layers
    self.n2_stride_conv = nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1)
    self.n3_stride_conv = nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1)
    self.n4_stride_conv = nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1)

    # process layers
    self.n3_process_conv = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
    self.n4_process_conv = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
    self.n5_process_conv = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
  ...
  def train(self, mode=True):
    ...
    if mode:
      ...
      self.n2_stride_conv.train()
      self.n3_stride_conv.train()
      self.n4_stride_conv.train()

      self.n3_process_conv.train()
      self.n4_process_conv.train()
      self.n5_process_conv.train()
