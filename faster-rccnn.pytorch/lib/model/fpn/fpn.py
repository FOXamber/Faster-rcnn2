...
class _FPN(nn.Module):
    ...
    def _init_weights(self):
        ...
        normal_init(self.n2_stride_conv, 0, 0.01, cfg.TRAIN.TRUNCATED)
        normal_init(self.n3_stride_conv, 0, 0.01, cfg.TRAIN.TRUNCATED)
        normal_init(self.n4_stride_conv, 0, 0.01, cfg.TRAIN.TRUNCATED)
        normal_init(self.n3_process_conv, 0, 0.01, cfg.TRAIN.TRUNCATED)
        normal_init(self.n4_process_conv, 0, 0.01, cfg.TRAIN.TRUNCATED)
        normal_init(self.n5_process_conv, 0, 0.01, cfg.TRAIN.TRUNCATED)
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        n2 = p2
        stride_n2 = self.n2_stride_conv(n2)
        stride_n2 = F.interpolate(stride_n2, p3.size()[2:], mode='bilinear', align_corners=True)
        n3 = self.n3_process_conv(stride_n2 + p3)

        stride_n3 = self.n3_stride_conv(n3)
        stride_n3 = F.interpolate(stride_n3, p4.size()[2:], mode='bilinear', align_corners=True)
        n4 = self.n4_process_conv(stride_n3 + p4)

        stride_n4 = self.n4_stride_conv(n4)
        stride_n4 = F.interpolate(stride_n4, p5.size()[2:], mode='bilinear', align_corners=True)
        n5 = self.n5_process_conv(stride_n4 + p5)


        # n3 = self.n3_process_conv((self.n2_stride_conv(n2) + p3))
        # n4 = self.n4_process_conv((self.n3_stride_conv(n3) + p4))
        # n5 = self.n5_process_conv((self.n4_stride_conv(n4) + p5))
        n6 = self.maxpool2d(n5)
